...
import os
import pandas as pd
import sqlalchemy  # Ensure the version is compatible
...
# Connection strings
try:
    USERNAME_PG = 'postgres'  # Specify your username
    PASSWORD_PG = 'postgres'  # Change this (keep it in a different .py file)
    HOST_PG = 'localhost'  # Server in Frankfurt / endpoint
    PORT_PG = 5432  # May not need to be specified
    ...
    # Create an engine
    engine = sqlalchemy.create_engine(conn_string_pg_render)
    connection = engine.connect()  # Connect using SQLAlchemy engine
    
    logging.critical("\n---- Successfully connected to database ----\n")
except Exception as e:
    logging.exception("\n---- Not connected to database ----\n")
    raise e  # Optionally raise the exception to see the full traceback
...
if nav == "Home":
    st.markdown("Compare your study choice and living expenses in England and Germany")
    st.header("Search your choice out of 650 English-speaking Bachelor and Master programs")
    ...
    sql_df1 = sql_df1.replace({"ae": "ä", "oe": "ö", "ue": "ü"}, regex=True)

    query_2 = """SELECT e.region,
    ...
    total_rent_de = german_yr_r_mean * (2 if option1 == "Master Degree" else 3)
    total_fee_de = german_yr_f_mean * (2 if option1 == "Master Degree" else 3)
    ...
    total_rent_en = english_yr_r_mean * (1 if option1 == "Master Degree" else 3)
    total_fee_en = english_yr_f_mean * (1 if option1 == "Master Degree" else 3)
    english_total_costs = total_rent_en + total_fee_en

    st.write("How much you will spend on your degree in Germany", total_rent_de, "pounds")
    st.write("How much you will spend on your degree in Germany", total_fee_de, "pounds")
    st.write("How much you will spend on your degree in Germany", german_total_costs, "pounds")
    st.write("How much you will spend on your degree in England", total_rent_en, "pounds")
    st.write("How much you will spend on your degree in England", total_fee_en, "pounds")
    st.write("How much you will spend on your degree in England", english_total_costs, "pounds")
    ...
    st.write("This much you are willing to pay", rent_en, 'pounds per semester. This is the uni options')
...
# Ensure the app runs on Streamlit
